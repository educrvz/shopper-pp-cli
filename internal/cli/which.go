...
var whichIndex = []whichEntry{
	...
	{Command: "payment methods", Description: "Reports live card, boleto, and PIX availability plus store-specific minimum lead times.", Group: "Customer journey plumbing", WhyItMatters: "Use immediately before checkout to select a payment path the current storefront actually supports."},
	{Command: "checkout prepare", Description: "Validates a selected payment method and basket readiness before handing final order confirmation to Shopper's browser.", Group: "Customer journey plumbing", WhyItMatters: "Use for one-time purchases where payment must be explicit instead of silently relying on a recurring saved card."},
}
